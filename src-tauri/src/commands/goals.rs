...
pub async fn get_goals(
    ...
    let goals: Vec<Goal> = DatabaseUtils::execute_query(&db, &final_query, where_params).await?;
...
pub async fn get_goal_by_id(goal_id: String, db: State<'_, Database>) -> Result<Goal, FiscusError> {
    ...
    let goal: Option<Goal> =
        DatabaseUtils::execute_query_single(&db, query, vec![Value::String(goal_id.clone())])
            .await?;

    goal.ok_or_else(|| FiscusError::NotFound("Goal not found".to_string()))
}
...
pub async fn update_goal(
    ...
    params.push(Value::String(goal_id.clone()));
...
pub async fn get_goal_progress_summary(
    ...
    let summary_query = r#"
        SELECT 
            ...
            COALESCE(AVG(CASE WHEN target_amount > 0 THEN (current_amount * 100.0 / target_amount) ELSE 0 END), 0) as average_progress_percentage
        FROM goals
        WHERE user_id = ?1
    "#;
